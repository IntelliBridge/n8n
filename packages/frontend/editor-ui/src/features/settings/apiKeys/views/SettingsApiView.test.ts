...
const assertHintsAreShown = ({ isSwaggerUIEnabled }: { isSwaggerUIEnabled: boolean }) => {
	...
	expect(apiDocsLink).toHaveAttribute('href', 'https://docs.flow.io/api');
	...
	expect(webhookDocsLink).toHaveAttribute(
		'href',
		'https://docs.flow.io/integrations/builtin/core-nodes/n8n-nodes-base.webhook/',
	);
	expect(webhookDocsLink).toHaveAttribute('target', '_blank');

	expect(
		screen.getByText('Use your API Key to control Flow programmatically using the', {
			exact: false,
		}),
	).toBeInTheDocument();
...
describe('SettingsApiView', () => {
	...
	it('if user public api is not enabled and user is trialing it should show upgrade call to action', () => {
		...
		expect(
			screen.getByText(
				'To prevent abuse, we limit API access to your workspace during your trial. If this is hindering your evaluation of Flow, please contact',
			),
		).toBeInTheDocument();
		expect(screen.getByText('support@flow.io')).toBeInTheDocument();
	...
	it('if user public api enabled and no API keys in account, it should create API key CTA', () => {
		...
		// Flow: the fork's i18n strips the docs link, so this renders as one text node
		expect(
			screen.getByText('Control Flow programmatically using the Flow API'),
		).toBeInTheDocument();
	});
	...
	it('should show delete warning when trying to delete an API key', async () => {
		...
		expect(
			screen.getByText(
				'Any application using this API Key will no longer have access to Flow. This operation cannot be undone.',
			),
		).toBeInTheDocument();
